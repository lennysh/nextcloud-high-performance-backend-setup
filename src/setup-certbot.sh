...
function certbot_step1() {
	...
	if ! is_dry_run; then
		...
		dnf install $args certbot python3-certbot-nginx 2>&1 | tee -a "$LOGFILE_PATH"
	else
		log "Would have installed certbot python3-certbot-nginx via dnf now."
	fi
}
...
function certbot_step2() {
	...
	if ! run_certbot_command && ! is_dry_run; then
		...
		if [ "$UNATTENDED_INSTALL" != true ]; then
			...
			)'/etc/nginx/conf.d/nextcloud-hpb.conf'? [YyNn]" -n 1 -r && echo
			if [[ $REPLY =~ ^[YyJj]$ ]]; then
				rm -v "/etc/nginx/conf.d/nextcloud-hpb.conf" |& tee -a $LOGFILE_PATH || true
	...
	log "Making SSL certificates readable by the nginx group."
	...
	is_dry_run || chown -R root:nginx /etc/letsencrypt/archive
	is_dry_run || chown -R root:nginx /etc/letsencrypt/live
