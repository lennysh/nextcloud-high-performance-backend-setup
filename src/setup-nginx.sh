...
function nginx_step1() {
	...
	if ! is_dry_run; then
		...
		dnf install $args nginx 2>&1 | tee -a "$LOGFILE_PATH"
	fi
}
...
function nginx_step2() {
	...
	log "Replacing '<EL_VERSION>' with '$EL_OS_VERSION_MAJOR'…"
	sed -i "s|<EL_VERSION>|$EL_OS_VERSION_MAJOR|g" "$TMP_DIR_PATH"/nginx/*
...
function nginx_step3() {
	...
	deploy_file "$TMP_DIR_PATH"/nginx/nextcloud-hpb.conf /etc/nginx/conf.d/nextcloud-hpb.conf || true
	...
	is_dry_run || find /var/www/html -maxdepth 1 -type f -name 'index.*html' -delete 2>/dev/null || true
